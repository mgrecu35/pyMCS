...
FC   = gfortran
OPTS = -ffree-form -ffree-line-length-none -O2 -finline-functions -fopenmp -fPIC
CPP  = cpp -C -P -traditional -Wno-invalid-pp-token -ffreestanding
OMP  = -DOPENMP
#-----------------------------------------------------------------------------
...
cm1:			$(OBJS)
			...
			$(AR) onefile.F $(SRC)
			mv onefile.F ../run
			gfortran -shared $(OBJS) $(FFLAGS) $(OUTPUTINC) $(OUTPUTLIB) $(LINKOPTS) -o ../run/cm1lib.dylib
